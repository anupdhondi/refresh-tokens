  ...
  try {
    ...
    if (!refreshToken) {
      ...
      if (!tokenDoc) {
        ...
        console.log("🚀 ~ file: AuthController.js ~ line 70 ~ exports.generateRefreshToken= ~ payload", payload);
        const accessToken = jwt.sign({ user: payload.user }, ACCESS_TOKEN_SECRET, {
          expiresIn: "5s",
        });
        let refreshToken = jwt.sign({ user: payload.user }, REFRESH_TOKEN_SECRET, {
          expiresIn: "1d",
        });
        ...
        // };
        // res.cookie("refreshToken", refreshToken, cookieOptions);
        return res.status(200).json({ accessToken, username: payload.user.username, refreshToken });
      }
    }
  } catch (error) {
    return res.status(500).json({ error: "Internal Server Error!" });
